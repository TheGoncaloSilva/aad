...
int main (int argc, char **argv)
{
  ...
  for (i = 0; i < N_SECTORS; i++)
    modify_sector_cpu_kernel (&host_sector_data[i], host_sector_number[i], n_sectors, sector_size);
                                      ...
                                      unsigned int sector_size)
{
 ...
 // modify the sector data
  ...
  { x = a * x + c;                            // update the pseudo-random generator state
    sector_data[i * n_sectors] ^= x;                      // modify the sector data
  }
}
